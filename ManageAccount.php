<?php 
	...
	switch ($flag) 
	{
		...
		case 'Login':
			...
			{
			$source->Query("SELECT * From `admin` WHERE  `email`='$email' and `password`='$password' ");
			$data=$source->Single();
			if($data){
			...
			if (empty($oldp) or empty($newp) or empty($conp)) {
				...
				if (sha1($oldp)==$data->password) {
					if ($newp==$conp) {
						...
						$query=$source->Query("UPDATE `user` SET `password`=?  WHERE `user_name`=?",[sha1($newp),$data->user_name]);
						$_SESSION['password']=$newp;
							$output = array('res' => 'success', 'msg'=>'Success! Your Password has been changed!','url'=>'Profile');
							echo json_encode($output);	
						}
					}else{
			...
			case 'UpdateProfileDetails':
				...
				if (empty($name)  or empty($email) or empty($mobile)) {
					...
					$query=$source->Query("UPDATE `admins` SET `name`=? ,`email`=? ,`mobile`=?  WHERE `name`=?",[$name,$email,$mobile,$uname]);
					$output = array('res' => 'success', 'msg'=>'Success! Your Profile has been changed!','url'=>'dashboard');
							echo json_encode($output);	
				}
				break;
				case 'UpdateProfile':
